...
// les voyage sont stockées dans $_SESSION
$voyage = $_SESSION['voyage'];
$montant = number_format($voyage['prix_total'], 2, '.', '');
$transaction_id = uniqid();
$vendeur = "MI-1_A";
$retour_url = "http://localhost/paiement.php"; // sa retourne vers le même fichier après la validation du paiement 
...
if (isset($_GET['status'])) {
    $status = $_GET['status'];//indique si le paiement est acceptee ou refuser 
    ...
    if ($status === "accepted") {
        ...
        $paiements = json_decode(file_get_contents('paiements.json'), true);
        $paiements[] = [
        ...
        //recaputilatif si le paiement à ete accepter
...
<body>
    ...
    <p><strong>Dates :</strong> <?= htmlspecialchars($voyage['dates']['debut']) . " - " . htmlspecialchars($voyage['dates']['fin']); ?></p>
